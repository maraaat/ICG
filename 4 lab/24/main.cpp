...
public:
    ...
    virtual void RenderSceneCB()
    {
        ...
        m_scale += 0.2f; //коэф масштабирования

        ShadowMapPass(); //проход для теней
        RenderPass(); //проход рендера
    ...
    virtual void ShadowMapPass()
    {
        ...
        //разрешаем метод отображения теней каждый раз
    ...
    virtual void RenderPass()
    {
        ...
        //заменяем метод теней на свет
        m_pLightingEffect->Enable();
        //привязываем карту теней для чтения в модуль текстур 1
        m_shadowMapFBO.BindForReading(GL_TEXTURE1);
        //рендерим плоскость так, что бы она служила землей, на которую падает тень
        Pipeline p;
        p.SetPerspectiveProj(60.0f, WINDOW_WIDTH, WINDOW_HEIGHT, 1.0f, 50.0f);
